...
| 파일 | 설명 |
|------|------|
| `middleware.ts` | `/admin`·`/api/admin` 접근 시 역할 검사(엣지 런타임). `@/lib/auth.config`만 임포트하며 **Prisma를 끌어들이면 배포가 실패한다** — `lib/AGENTS.md` 참고 |
...
- `middleware.ts`가 (직간접적으로) 임포트하는 파일에는 Prisma를 넣지 말 것 — 엣지 번들 1MB 제한
